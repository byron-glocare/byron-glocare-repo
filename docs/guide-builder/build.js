...
(async () => {
  ...
  c.push(H("4.4.1 자동으로 채워지는 항목", 3));
  c.push(P("이미 받은 정보는 다시 치지 않아도 됩니다. 정보 입력 화면을 열면 아래 항목이 자동으로 들어가 있습니다."));
  c.push(table(
    ["어디서 온 값인가", "자동으로 채워지는 항목"],
    [
      ["학생을 등록할 때 입력한 값", "이름 · 전화번호 · 이메일 · 생년월일 · TOPIK 급수"],
      ["지원 대학을 선택하면 정해지는 값", "지원 학과 · 지원 년도 · 지원 학기 · 지원 월"],
    ],
    [40, 60]
  ));
  c.push(note("자동으로 채워져도 고칠 수 있습니다", [
    "값이 마음에 들지 않으면 그냥 칸을 수정하면 됩니다.",
    "이미 입력해 둔 칸은 자동 채움이 건드리지 않습니다. 화면을 다시 열어도 고쳐 놓은 값이 그대로 남습니다.",
    "복수 지원인 경우 가장 먼저 등록한 지원을 기준으로 채워집니다. 다른 대학 기준이 필요하면 직접 고치세요.",
  ], "0F766E"));

  c.push(H("4.4.2 추천인 — 입력하지 않습니다", 3));
  c.push(P("추천인은 항상 '글로케어'로 들어갑니다. 담당자가 입력할 항목이 아니라서 입력칸 대신 '자동 입력 항목' 안내로만 보입니다."));
  c.push(note("입력칸이 안 보인다고 빠진 게 아닙니다", [
    "서류에는 정상적으로 '글로케어'가 채워집니다.",
    "사람이 칠 이유가 없는 값이라 오타가 날 여지를 없앤 것입니다.",
  ], "0F766E"));

  c.push(H("4.4.3 번역 — [KR 번역] / [EN 번역] 버튼", 3));
  ...
  c.push(H("4.4.4 도구 — 학생에게 직접 입력 링크 보내기", 3));
  ...
  // ── 5. 부록 ──────────────────────────────────────────
  c.push(H("5. 학생이 직접 신청하는 서비스", 1));
  c.push(P("학생이 글로케어 홈페이지에서 직접 신청하고 결제하는 서비스가 있습니다. 센터를 거치지 않는 경로라 담당자가 알고 안내만 하면 됩니다."));
  c.push(...await image("s10_service", "서비스 안내 — 홈페이지의 판매 상품"));
  c.push(table(
    ["상품", "금액", "포함 내용"],
    [
      ["서류 발급 풀세트", "150,000원", "졸업·성적증명서 발급 대행, 한국어 번역, 공증, 영사확인, 완료 서류 전달"],
      ["유학 준비 종합 컨설팅", "600,000원", "위 전 항목 + 지원 대학·학과 상담, 모집요강별 서류 준비, 지원서·자기소개서 준비 지원, 최종 점검"],
    ],
    [26, 16, 58]
  ));
  c.push(...await image("s20_service_detail", "상품 상세 — 포함 내역과 서비스 제공기간"));
  c.push(...steps([
    "학생이 홈페이지에서 [유학 지원]으로 들어가 계정을 만듭니다.",
    "[서비스 · 요금]에서 상품을 고르고 [자세히 보고 신청]을 누릅니다.",
    "주문 내용을 확인하고 카드로 결제합니다.",
    "결제가 끝나면 담당자가 배정되어 진행 상황을 안내합니다.",
  ]));
  c.push(note("담당자가 알아 두면 좋은 것", [
    "· 금액은 모두 부가세가 포함된 가격입니다.",
    "· 서비스 제공기간은 결제일로부터 최대 3개월입니다.",
    "· 취소·환불은 진행 단계에 따라 달라집니다. 홈페이지 하단 [취소·환불 규정]에서 확인할 수 있습니다.",
    "· 이 결제는 학생 본인이 직접 하는 것이며, 센터 청구서(Hóa đơn)와는 별개입니다.",
  ], "0F766E"));

  c.push(H("6. 부록", 1));
  c.push(H("6.1 권장 작업 순서", 2));
  c.push(...steps([
    "학생 등록 (이름은 여권과 동일하게)",
