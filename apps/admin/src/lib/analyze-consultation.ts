...
### 3c. suggestions.reminders (챙길 일정 후보, 최대 5개)

상담에서 **특정 날짜에 후속해야 할 항목**이 명확히 언급된 경우만 추출.
"5월 20일에 면접 결과 확인 필요", "다음 주 월요일에 비자 서류 받기로 함" 처럼
**날짜 + 챙길 내용** 둘 다 명시되어야 함. 모호하거나 막연한 약속은 제외.

- remind_date: "YYYY-MM-DD" 형식. 상담의 문맥상 날짜를 절대 날짜로 변환.
  - "다음 주 월요일" 처럼 상대 표현이면 — 오늘 날짜 모르므로 변환 불가 → 제외.
  - "5월 20일" 같이 연도가 빠진 경우 — 가장 가까운 미래 연도 추정 (한 달 이내).
- content: 챙길 항목을 짧고 명확한 한국어 명사구로 (예: "면접 결과 확인",
  "비자 갱신 서류 받기", "예약금 입금 확인"). 30자 이내 권장.
